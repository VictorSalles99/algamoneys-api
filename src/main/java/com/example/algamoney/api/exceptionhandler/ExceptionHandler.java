...
public class ExceptionHandler extends ResponseEntityExceptionHandler{
	...
	protected ResponseEntity<Object> handleHttpMessageNotReadable(HttpMessageNotReadableException ex,
		...
		//pego o valor da mensagem de erro informada no arquivo de definições de mensagens para tratativas
		String retornoDeErroUsuario = messageSource.getMessage("mensagem.invalida", null, LocaleContextHolder.getLocale());
		
		//pego a causa da exeção (ex.getCause())
		String retornoDeErroDev = ex.getCause().toString();
		
		return handleExceptionInternal(ex, new Erro(retornoDeErroUsuario, retornoDeErroDev), headers, HttpStatus.BAD_REQUEST, request);
	}
	
	// Essa nova classe foi gerada para retornar as mensagens do user e do dev juntamente
