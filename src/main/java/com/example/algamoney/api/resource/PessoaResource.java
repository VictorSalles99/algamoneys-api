...
public class PessoaResource {
	...
	public ResponseEntity<?> criarPessoa(@Valid @RequestBody Pessoa pessoa, HttpServletResponse response) {
		...
		publisher.publishEvent(new RecursoEvents(this, response, pessoaSalva.getCodigo()));
	...
	public ResponseEntity<Pessoa> buscarPorId(@PathVariable Long codigo) {
		...
		return !pessoa.isEmpty() ? ResponseEntity.ok(pessoa.orElse(null)) : ResponseEntity.notFound().build();
	}
	...
	@ResponseStatus(HttpStatus.NO_CONTENT)
	public void atualizarStatusPessoa(@PathVariable Long codigo, @RequestBody Boolean status) {
		pessoaService.atualizarStatusPessoa(codigo, status);	
	}
}
